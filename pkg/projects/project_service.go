...
func (s *ProjectService) GetResourceManifest(request *resources.GetResourceManifestRequest) (*resources.GetResourceManifestResponse, error) {
	if request == nil {
	...
	resp, err := api.ApiGet(s.GetClient(), new(resources.GetResourceManifestResponse), path)
	if err != nil {
		return nil, err
	}

	return resp.(*resources.GetResourceManifestResponse), nil
}
...
// GetResourceManifest retrieves a resource manifest using the new client implementation
func GetResourceManifest(client newclient.Client, request *resources.GetResourceManifestRequest) (*resources.GetResourceManifestResponse, error) {
	if request == nil {
		return nil, internal.CreateInvalidParameterError("GetResourceManifest", "request")
	}
	...
	resp, err := newclient.Get[resources.GetResourceManifestResponse](client.HttpSession(), expandedUri)
	if err != nil {
		return nil, err
	}
