...
func createCertificate(t *testing.T) (*CertificateResource, error) {
	...
	certificateData := core.NewSensitiveValue(certificate)
	require.NotNil(t, certificateData)

	password := core.NewSensitiveValue(certificatePassword)
	require.NotNil(t, password)

	resource := NewCertificateResource(internal.GetRandomName(), certificateData, password)
...
func createCertificateService(t *testing.T) *CertificateService {
	service := NewCertificateService(nil, constants.TestURICertificates)
	services.NewServiceTests(t, service, constants.TestURICertificates, constants.ServiceCertificateService)
	return service
}
...
func TestCertificateServiceNew(t *testing.T) {
	...
	client := &sling.Sling{}
	uriTemplate := ""
	ServiceName := constants.ServiceCertificateService
...
func TestCertificateServiceReplace(t *testing.T) {
	...
	assert.Equal(t, err, internal.CreateInvalidParameterError("Replace", "certificateID"))
	...
	assert.Equal(t, err, internal.CreateInvalidParameterError("Replace", "replacementCertificate"))
	assert.Nil(t, certificate)
