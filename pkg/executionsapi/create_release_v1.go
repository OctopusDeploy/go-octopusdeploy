...
func CreateReleaseV1(client *client.Client, command *CreateReleaseCommandV1) (*CreateReleaseResponseV1, error) {
	if command == nil {
	...
	resp, err := services.ApiPost(client.Root.GetClient(), command, new(CreateReleaseResponseV1), url)
	if err != nil {
		return nil, err
	}
