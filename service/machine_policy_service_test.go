...
func CreateTestMachinePolicy(t *testing.T, service *machinePolicyService) *MachinePolicy {
	...
	connectionConnectTimeout := octopusdeploy.getRandomDuration(0)
	connectionRetrySleepInterval := octopusdeploy.getRandomDuration(0)
	connectionRetryTimeLimit := octopusdeploy.getRandomDuration(0)
	pollingRequestMaximumMessageProcessingTimeout := octopusdeploy.getRandomDuration(0)
	pollingRequestQueueTimeout := octopusdeploy.getRandomDuration(0)

	machineCleanupPolicy := NewMachineCleanupPolicy()
	machineCleanupPolicy.DeleteMachinesElapsedTimeSpan = octopusdeploy.getRandomDuration(0)

	machineHealthCheckPolicy := NewMachineHealthCheckPolicy()
	machineHealthCheckPolicy.HealthCheckInterval = octopusdeploy.getRandomDuration(0)
...
func IsEqualMachinePolicies(t *testing.T, expected *MachinePolicy, actual *MachinePolicy) {
	...
	assert.True(t, octopusdeploy.IsEqualLinks(expected.GetLinks(), actual.GetLinks()))
...
func TestMachinePolicyServiceGetByID(t *testing.T) {
	...
	id := octopusdeploy.getRandomName()
