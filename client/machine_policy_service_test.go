...
func CreateTestMachinePolicy(t *testing.T, service *machinePolicyService) *model.MachinePolicy {
	if service == nil {
	...
	name := getRandomName()
	connectionConnectTimeout := getRandomDuration()
	connectionRetrySleepInterval := getRandomDuration()
	connectionRetryTimeLimit := getRandomDuration()
	pollingRequestMaximumMessageProcessingTimeout := getRandomDuration()
	pollingRequestQueueTimeout := getRandomDuration()

	machineCleanupPolicy := model.NewMachineCleanupPolicy()
	machineCleanupPolicy.DeleteMachinesElapsedTimeSpan = getRandomDuration()

	machineHealthCheckPolicy := model.NewMachineHealthCheckPolicy()
	machineHealthCheckPolicy.HealthCheckInterval = getRandomDuration()

	machinePolicy := model.NewMachinePolicy(name)
...
func IsEqualMachinePolicies(t *testing.T, expected *model.MachinePolicy, actual *model.MachinePolicy) {
...
func UpdateMachinePolicy(t *testing.T, service *machinePolicyService, machinePolicy *model.MachinePolicy) *model.MachinePolicy {
	if service == nil {
		service = createMachinePolicyService(t)
	}
	require.NotNil(t, service)
...
func TestMachinePolicySetAddGetDelete(t *testing.T) {
	...
	resource, err = service.Add(&model.MachinePolicy{})
	require.Error(t, err)
	require.Nil(t, resource)
...
func TestMachinePolicyServiceUpdate(t *testing.T) {
	...
	resource, err = service.Update(&model.MachinePolicy{})
	require.Error(t, err)
	require.Nil(t, resource)
