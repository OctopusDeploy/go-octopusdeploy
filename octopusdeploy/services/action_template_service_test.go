...
func createActionTemplate(t *testing.T) *resources.ActionTemplate {
	...
	resource.Properties[ActionTypeOctopusActionScriptBody] = NewPropertyValue(octopusdeploy.getRandomName(), false)
...
func TestActionTemplateServiceAdd(t *testing.T) {
	...
	assert.Equal(t, octopusdeploy.createValidationFailureError("Add", invalidResource.Validate()), err)
	assert.Nil(t, resource)
