...
func createCertificate(t *testing.T) (*CertificateResource, error) {
	...
	resource := NewCertificateResource(octopusdeploy.getRandomName(), certificateData, password)
...
func TestCertificateServiceAdd(t *testing.T) {
	...
	require.Equal(t, octopusdeploy.createValidationFailureError("Add", invalidResource.Validate()), err)
	require.Nil(t, resource)
