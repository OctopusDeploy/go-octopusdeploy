...
func New_GetTestDeploymentProcess(t *testing.T, client *client.Client, project *projects.Project) *deployments.DeploymentProcess {
	if client == nil {
		client = getOctopusClient()
	}
	require.NotNil(t, client)

	deploymentProcess, err := deployments.GetDeploymentProcessByID(client, client.GetSpaceID(), project.DeploymentProcessID)
	require.NotNil(t, deploymentProcess)
	require.NoError(t, err)
	require.Equal(t, project.DeploymentProcessID, deploymentProcess.GetID())
...
func New_GetByGitRefTestDeploymentProcess(t *testing.T, client *client.Client, project *projects.Project, gitRef string) *deployments.DeploymentProcess {
	if client == nil {
		client = getOctopusClient()
	}
	require.NotNil(t, client)

	deploymentProcess, err := deployments.GetDeploymentProcessByGitRef(client, client.GetSpaceID(), project, gitRef)
	require.NotNil(t, deploymentProcess)
	require.NoError(t, err)
	require.Equal(t, project.DeploymentProcessID, deploymentProcess.GetID())
...
func New_TestDeploymentProcessGet(t *testing.T) {
	client := getOctopusClient()
	require.NotNil(t, client)
	...
	// TODO: Convert this to new spaceID GetChannels
	channels, err := client.Projects.GetChannels(project)
	require.NotNil(t, channels)
	require.NoError(t, err)

	template, err := deployments.GetDeploymentProcessTemplate(client, deploymentProcess, channels[0].GetID(), "")
	require.NoError(t, err)
	require.NotNil(t, template)
}

func New_TestDeploymentProcessGetAll(t *testing.T) {
	client := getOctopusClient()
	space := GetDefaultSpace(t, client)
	require.NotNil(t, client)

	deploymentProcesses, err := deployments.GetAllDeploymentProcesses(client, space.ID)
	require.NoError(t, err)
	...
	totalDeploymentProcesses, err := deployments.GetAllDeploymentProcesses(client, space.ID)
	require.NoError(t, err)

	assert.Equal(t, len(totalDeploymentProcesses), numberOfDeploymentProcesses+1, "created an additional project and expected number of deployment processes to increase by 1")
}

func New_TestDeploymentProcessUpdate(t *testing.T) {
	client := getOctopusClient()
	require.NotNil(t, client)
